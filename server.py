...
player_spawns = ['4,4', '96,4', '96,96', '4,96']
...
def genPowerupSpawns():
    powerup_spawns = []
    while len(powerup_spawns) < 6:
        x = random.randint(20, 80)
        y = random.randint(20, 80)
        if map[x, y] == 2 and f"{x},{y}" not in powerup_spawns:
            powerup_spawns.append(f"{x},{y}")
...
def handleMessage(message, client_addr):
    ...
    if message == check_start_msg:
        if len(player_states) == 4:
            ...
            first_transmission = f"{players}-{powerup_spawns}-{player_spawns[0]}"
            player_spawns.pop(0)

            server.sendto(first_transmission.encode(format), client_addr)
        else:
...
while True:
    message, client_addr = server.recvfrom(65535)
