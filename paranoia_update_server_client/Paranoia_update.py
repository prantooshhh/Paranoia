...
def sendCheckStart():
    ...
    if data != not_start_msg:
        start = True
        data = data.split()
        pw_coords, _ = client.recvfrom(1024)
        
        p

        for player in data:
            if player != ip_addr:
                player_states[player] = {'x': player.x,
                                      'y': player.y,
                                      'state': 'alive',
...
def sendrecvUpdate():
    ...
    for player, info in update_recv.items():
        if player != ip_addr:
            ...
            player_states[player]['killed'] = info['killed']
        if player == ip_addr:
            if player_states[player]['state'] == 'dead':
                player.state = 'dead'
                game_state['mode'] = 'over'
