...
GRID_LENGTH = 50        # 50 for more natural lighting, pc may go boom tho
...
def colorFunc(x, y, r, g, b):
    ...
    if dist > view_range:
        ...
        dx = math.sin(math.radians(player.angle))
        dy = -math.cos(math.radians(player.angle))
        dlen = math.sqrt(dx**2 + dy**2)
        dx, dy = dx/dlen, dy/dlen

        dot = vx*dx + vy*dy

        angle_fac = max(0, (dot - math.cos(math.radians(view_angle))) / (1 - math.cos(math.radians(view_angle))))
        dist_fac = 1 - (dist / view_range)
...
def gridBlock(x, y):
    ...
    glColor3f(*colorFunc(x+grid_mid, y-grid_mid, 1, 1, 1))
    ...
    glColor3f(*colorFunc(x-grid_mid, y-grid_mid, 0.7, 0.5, 0.95))
...
def grid():
    ...
    for i in range(-grid_size//2, grid_size//2+1, GRID_LENGTH*2):
        for j in range(-grid_size//2, grid_size//2+1, GRID_LENGTH*2):
            gridBlock(i,j)
